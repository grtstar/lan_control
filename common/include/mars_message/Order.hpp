...
class Order
{
    ...
    public:
        NLOHMANN_DEFINE_TYPE_INTRUSIVE(Order, orderId, sign, valid, time, cleanNum, mode, sweepPower, mopPower, componentCount, roomCount, components, rooms)
#endif
#ifdef __MSGPACK
    public:
        MSGPACK_DEFINE(Order, orderId, sign, valid, time, cleanNum, mode, sweepPower, mopPower, componentCount, roomCount, components, rooms)
#endif
    public:
        bool operator==(const Order& other) const
        {
            return orderId == other.orderId && sign == other.sign && valid == other.valid && time == other.time && cleanNum == other.cleanNum && mode == other.mode && sweepPower == other.sweepPower && mopPower == other.mopPower && componentCount == other.componentCount && roomCount == other.roomCount && components == other.components && rooms == other.rooms;
        }
    public:
        /**
...
int Order::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    for (int a0 = 0; a0 < this->roomCount; a0++) {
...
int Order::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    try {
        this->components.resize(this->roomCount);
    } catch (...) {
        return -1;
    }
    for (int a0 = 0; a0 < this->roomCount; a0++) {
...
uint64_t Order::_computeHash(const __lcm_hash_ptr *p)
{
    ...
    uint64_t hash = 0xe9bfbbbfb010267cLL +
